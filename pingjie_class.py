    ...
    def tuiduan_img(self, img, sess1):
        ...
        for x in range(20):
            if score_list[0][x] > 0.6 and class_list[0][x] == 1.0:
                ...
                class_id = class_list[0][x]
                points_list.append([point[1], point[0], point[3], point[2], score,class_id])
        # points_list = []
        # print(points_list)
        return points_list
    ...
    def pingjie(self):
        ...
        for x_l in range(h_num):
            for y_l in range(w_num):
                ...
                print("处理480*480需要时间 {}".format(time.time()-t0))
                if points_list:
        ...
        #     # 绘制最终拼接的检测结果
        #     all_last_result_points.append([int(last_point[0]), int(last_point[1]), int(last_point[2]), int(last_point[3])])
